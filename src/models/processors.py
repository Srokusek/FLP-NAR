...
class combined_processor(nn.Module):
    def __init__(self, hidden_dim, mixing: bool = True):
        ...
        self.out = HeteroConv({
            ...
            ("alpha", "to", "y"): SAGEConv((hidden_dim, hidden_dim), hidden_dim, aggr=aggr.MultiAggregation(["min", "sum"])),
        })
    ...
    def forward(self, graph: HeteroData, masks: dict):
        ...
        if masks is not None:
            ...
            edge_index_dict[("y", "to", "beta")] = edge_index_dict[("y", "to", "beta")][:,masks["active_clients"]]
            edge_index_dict[("beta", "to", "y")] = edge_index_dict[("beta", "to", "y")][:,masks["active_clients"]]
...
class simple_processor(nn.Module):
    def __init__(self, hidden_dim, mixing: bool = True):
        ...
        #step 3: alpha + beta to x, beta to y
        self.out = HeteroConv({
            ("alpha", "to", "x"): SAGEConv(hidden_dim, hidden_dim, aggr="mean"),
            ("beta", "to", "x"): SAGEConv(hidden_dim, hidden_dim, aggr=aggr.MultiAggregation(["min", "sum"])),
            ("beta", "to", "y"): SAGEConv(hidden_dim, hidden_dim, aggr=aggr.MultiAggregation(["min", "sum"])),
            ("alpha", "to", "y"): SAGEConv(hidden_dim, hidden_dim, aggr=aggr.MultiAggregation(["min", "sum"])),
            ("x", "to", "y"): SAGEConv(hidden_dim, hidden_dim, aggr=aggr.MultiAggregation(["min", "sum"])),
            ("y", "to", "x"): SAGEConv(hidden_dim, hidden_dim, aggr=aggr.MultiAggregation(["min", "sum"])),
        })
    ...
    def forward(self, graph: HeteroData, masks: dict):
        ...
        if masks is not None:
            ...
            # edge_index_dict[("y", "to", "beta")] = edge_index_dict[("y", "to", "beta")][:,masks["active_clients"]]
            edge_index_dict[("beta", "to", "y")] = edge_index_dict[("beta", "to", "y")][:,masks["active_clients"]]
            edge_index_dict[("x", "to", "y")] = edge_index_dict[("x", "to", "y")][:,masks["active_clients"]]
            edge_index_dict[("y", "to", "x")] = edge_index_dict[("y", "to", "x")][:,masks["active_clients"]]
